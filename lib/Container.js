...
function Container(props) {
    ...
    const lastModalRefCreate = useMemo(() => (ref) => { lastModalRef.current = ref; }, []);
    ...
    useCode(() => {
        if (toggleStatusBarStyle !== false) {
            ...
            return block([
                cond(and(greaterOrEq(scale, _threshold), neq(statusDark, 1)), [set(statusDark, 1), call([], () => {
                        StatusBar.setBarStyle('dark-content', true);
                    })]),
                cond(and(lessOrEq(scale, _threshold - 0.005), neq(statusDark, 0)), [set(statusDark, 0), call([], () => {
    ...
    return (<View style={styles.container}>
            <Animated.View style={{ flex: 1, overflow: 'hidden', borderRadius, transform: [{ scale }] }}>
                {children}
