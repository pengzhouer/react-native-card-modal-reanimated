...
export type LastModalRef = {closeLast: () => void}
type Props = {
    name: string,
    title: string | ((params: any) => string);
    transition: Animated.Value<number>;
    index: number; // the index layer of the modal
    component: any;
    isLast: boolean;
...
function Modal(props: Props){
    ...
    // first open
    const firstOpen: Animated.Value<0 | 1> = useMemo(() => new Value(1), [ready]);

    // did focus animated value
    const didFocusV: Animated.Value<0 | 1> = useMemo(() => new Value(0), []);
    // did focus callback ref
    const didFocusCallbackRef = useRef<(() => void) | null>();

    // on closing the modal
    const onClosing: Animated.Value<0 | 1> = useMemo(() => new Value(0), []);
    ...
    // spring state
    const springState: Animated.SpringState = useMemo(() => ({
    ...
    const modalNavigation: ModalNavigation = useMemo(() => ({
        params,
        setParams,
        close: () => {
            if(isLast){
    ...
    useCode(() => {
        if(isLast && ready){
            ...
            return block([
                cond(
                    and(firstOpen, neq(state, State.ACTIVE)),
                    [
                ...
                cond(
                    and(onClosing, neq(state, State.ACTIVE)),
                    [
                ...
                cond(eq(state, State.ACTIVE), [
                    ...
                    set(
                        translateY, 
                        interpolate(add(springState.position, translationY), {
                ...
                cond(
                    and(greaterOrEq(translateY, viewportHeight), canClose), 
                    [
                        stopClock(closeClock),
                        stopClock(snapClock),
    ...
    useCode(() => block([
        cond(
            and(lessOrEq(translateY, _top), not(didFocusV)),
            [
                set(didFocusV, 1),
                call([], () => {
    ...
    return (
        <Animated.View style={[StyleSheet.absoluteFill, styles.modal, {transform: [{translateY, scale}]}]}>
            {!hideHeader && (
